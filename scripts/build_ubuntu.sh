...
case "${OS_NAME}" in
	"Linux Mint")
		if [ "${OS_MAJ}" -lt 18 ]; then
			printf "You must be running Linux Mint 18.x or higher to install LeoPays.\\n"
			printf "Exiting now.\\n"
			exit 1
		fi
	;;
	"Ubuntu")
		if [ "${OS_MAJ}" -lt 16 ]; then
			printf "You must be running Ubuntu 16.04.x or higher to install LeoPays.\\n"
			printf "Exiting now.\\n"
			exit 1
		fi
	...
	"Debian")
		if [ $OS_MAJ -lt 10 ]; then
			printf "You must be running Debian 10 to install LeoPays, and resolve missing dependencies from unstable (sid).\n"
			printf "Exiting now.\n"
			exit 1
	fi
	;;
...
if [ "${DISK_AVAIL%.*}" -lt "${DISK_MIN}" ]; then
	printf "You must have at least %sGB of available storage to install LeoPays.\\n" "${DISK_MIN}"
	printf "Exiting now.\\n"
	exit 1
fi
...
if [ "${COUNT}" -gt 1 ]; then
	printf "\\nThe following dependencies are required to install LeoPays.\\n"
	printf "\\n${DISPLAY}\\n\\n" 
