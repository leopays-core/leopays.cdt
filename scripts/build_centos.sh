...
if [ "${OS_VER}" -lt 7 ]; then
	printf "\\nYou must be running Centos 7 or higher to install LeoPays.\\n"
	printf "Exiting now.\\n\\n"
	exit 1;
fi

if [ "${DISK_AVAIL%.*}" -lt "${DISK_MIN}" ]; then
	printf "\\nYou must have at least %sGB of available storage to install LeoPays.\\n" "${DISK_MIN}"
	printf "Exiting now.\\n\\n"
	exit 1;
fi
...
if ! YUM=$( command -v yum 2>/dev/null ); then
		printf "!! Yum must be installed to compile ELeoPays !!\\n"
		printf "Exiting now.\\n"
		exit 1;
fi
