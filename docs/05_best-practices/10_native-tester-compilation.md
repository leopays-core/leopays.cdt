...
As of v1.5.0 native compilation can be performed and a new set of libraries to facilitate native testing and native "scratch pad" compilation. [`leopays-cc`](../03_command-reference/cc.md), [`leopays-cpp`](../03_command-reference/cpp.md) and [`leopays-ld`](../03_command-reference/ld.md) now support building "smart contracts" and unit tests natively for quick tests to help facilitate faster development \(note the default implementations of eosio `intrinsics` are currently asserts that state they are unavailable, these are user definable.\)
