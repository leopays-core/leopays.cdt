...
In order to be able to debug your smart contract, you will need to setup a local leopays-node node. This local leopays-node node can be run as separate private testnet or as an extension of a public testnet.  This local node also needs to be run with the contracts-console option on, either `--contracts-console` via the command line or `contracts-console = true` via the config.ini and/or by setting up logging on your running leopays-node node and checking the output logs. See below for details on logging.

When you are creating your smart contract for the first time, it is recommended to test and debug your smart contract on a private testnet first, since you have full control of the whole blockchain and can easily add suitable logging. This enables you to have unlimited amount of LPC needed and you can just reset the state of the blockchain whenever you want. When it is ready for production, debugging  on the public testnet (or official testnet) can be done by connecting your local leopays-node to the public testnet (or official testnet) so you can see the log of the testnet in your local leopays-node.

The concept is the same, so for the following guide, debugging on the private testnet will be covered.

If you haven't set up your own local leopays-node, follow the setup guide. By default, your local leopays-node will just run in a private testnet unless you modify the config.ini file to connect with public testnet (or official testnet) nodes. 
...
```bash
$ leopays-cpp -abigen debug.cpp -o debug.wasm
$ leopays-cli set contract debug CONTRACT_DIR/debug -p youraccount@active
$ leopays-cli push action debug foo '{"from":"inita", "to":"initb", "amount":10}' --scope debug
```

When you check your local `leopays-node` node log, you will see the following lines after the above message is sent.
